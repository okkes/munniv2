 ...
 * The cockpit authenticates against the DESIGNATED environment's Logto
 * (admin split LS6) — the shared stack has no identity provider of its own. */
function Root() {
  if (!logtoConfigured) return <ControlApp config={config} getToken={null} />;
  return (
    <LogtoProvider
      config={{
...
// Single-flight token fetch: reload() fires several /control/* calls in
